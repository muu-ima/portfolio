...
実際の元運用では、古物台帳、EC販売、仕入れ表、為替レート、オーダー、ペイメント、仕入れ元データがタブで分かれ、手動入力と自動計算が同じシート上に混在していました。VLOOKUP や複数タブ参照に依存するため、`#N/A`、`#VALUE!`、未入力セル、手動補正を人が見ながら判断する必要がありました。

ポートフォリオでは実シートを出さず、横長シートで人が判断していた運用を、原票・商品・仕入れ・販売・精算に分けた Web 管理へ移したことを中心に伝えます。
